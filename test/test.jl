...
const sim_params = SimParams(number_agents=20, memory_length=10, memory_init_state=:fractious, error=0.1, tag1=:red, tag2=:blue, tag1_proportion=1.0, random_seed=1234)
...
@timeit test_times "makeChoices" begin
    ...
    if rand() <= sim_params.error
        ...
        @timeit test_times "player_choices" begin
...
function makeChoices_old(game::Game, sim_params::SimParams, players::Tuple{Agent, Agent}) #COULD LIKELY MAKE THIS FUNCTION BETTER. Could use CartesianIndices() to iterate through payoff matrix? 
    player_choices::Vector{Int8} = [rand(game.strategies[1]), rand(game.strategies[2])] #could change to MVector?
    if rand() <= sim_params.error
        return player_choices
    else
