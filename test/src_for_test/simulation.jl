...
function makeChoices!(players::Tuple{Agent, Agent}, game::Game, error) #COULD LIKELY MAKE THIS FUNCTION BETTER. Could use CartesianIndices() to iterate through payoff matrix? 
    ...
    function findOpponentStrategyProbs(player_memory, opponent_tag, opponent_strategies)
        @timeit times "findOpponentStrategyProbs()" begin #***
    ...
    @timeit times "findExpectedUtilities!()" findExpectedUtilities!(player_expected_utilities, game.payoff_matrix, findOpponentStrategyProbs(players[1].memory, players[2].tag, axes(game.payoff_matrix, 2)), findOpponentStrategyProbs(players[2].memory, players[1].tag, axes(game.payoff_matrix, 1)))
    ...
    function findMaximumStrats(expected_utilities::Vector{Float32})
        @timeit times "findMaximumStrats()" begin #***
    ...
    function choose!(players, strategies, error)
        @inbounds for player in eachindex(players)
            if rand() <= error
                players[player].choice = rand(strategies[player])
            else
                players[player].choice = findMaximumStrats(player_expected_utilities[player]) #if rand() <= error, do nothing (i.e., keep random choice)
            end
        end
        return nothing
    end

    @timeit times "choose!()" choose!(players, game.strategies, error)
...
function updateMemories!(players::Tuple{Agent, Agent}, memory_length)
    for player in players
        if length(player.memory) == memory_length
            popfirst!(player.memory)
        end
    end
    @inbounds push!(players[1].memory, (players[2].tag, players[2].choice))
    @inbounds push!(players[2].memory, (players[1].tag, players[1].choice))
    return nothing
end

#play the defined game
function playGame!(game::Game, sim_params::SimParams, players::Tuple{Agent, Agent})
    @timeit times "makeChoices!()" makeChoices!(players, game, sim_params.error)
    @timeit times "updateMemories!()" updateMemories!(players, sim_params.memory_length)
    return nothing
end
...
function runMatches!(meta_graph, game, sim_params)
    for match in 1:sim_params.matches_per_period
        @timeit times "edge collection" edge = rand(collect(edges(meta_graph))) #get random edge
        ...
        @timeit times "playGame!()" playGame!(game, sim_params, players)
    end
end
...
function simulateTransitionTime(game::Game, sim_params::SimParams, graph_params::GraphParams; periods_elapsed::Int128 = Int128(0), use_seed::Bool = false, db_filepath::Union{String, Nothing} = nothing, db_store_period::Union{Integer, Nothing} = nothing, db_sim_group_id::Union{Integer, Nothing} = nothing, prev_simulation_id::Union{Integer, Nothing} = nothing)
    ...
    @timeit times "while loop" begin #***
    while !checkTransition(meta_graph, game, sim_params)
        #play a period worth of games
        @timeit times "match for loop" begin #***
        runMatches!(meta_graph, game, sim_params)
        end #***
        periods_elapsed += 1
