...
function runPeriod!(model::SimModel, to::TimerOutput) #NOTE: what type are graph_edges ??
    ...
    for match in 1:model.sim_params.matches_per_period
        ...
        @timeit to "setPlayers" setPlayers!(model)
        #println(players[1].name * " playing game with " * players[2].name)
        @timeit to "playGame" playGame!(model, to)
        end
    end
    return nothing
    end
...
#play the defined game
function playGame!(model::SimModel, to::TimerOutput)
    # @timeit to "make choices" makeChoices!(model)
    # @timeit to "update memories" updateMemories!(model.pre_allocated_arrays.players, model.sim_params)
    @timeit to "updateMemories" begin
    updateMemory!(model.pre_allocated_arrays.players[1], model.pre_allocated_arrays.players[2].choice, model.sim_params)
    updateMemory!(model.pre_allocated_arrays.players[2], model.pre_allocated_arrays.players[1].choice, model.sim_params)
    end
    @timeit to "updateChoices" begin
    for player in model.pre_allocated_arrays.players
        updateChoice!(player, model.game, model.sim_params, model.pre_allocated_arrays, to) #update expected behavior and choice right after new percept is pushed to memory
        resetArrays!(model)
    end
    end
    return nothing
...
function updateMemory!(agent::Agent, percept::Percept, sim_params::SimParams)
...
function checkStoppingCondition(stopping_condition::EquityBehavioral, agent_graph::AgentGraph, ::Int128) #game only needed for behavioral stopping conditions. could formulate a cleaner method for stopping condition selection!!
    ...
    for agent in agent_graph.agents
        if !agent.is_hermit
            if agent.expected_behavior == stopping_condition.strategy #if the agent is acting in an equitable fashion (if all agents act equitably, we can say that the behavioral equity norm is reached (ideally, there should be some time frame where all or most agents must have acted equitably))
                number_transitioned += 1
            end
        end
    end 
