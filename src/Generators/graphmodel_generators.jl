...
struct SmallWorldModelGenerator <: GraphModelGenerator
    ...
    SmallWorldModelGenerator(λ::Vector{<:Real}, β::Vector{Float64}) = new(λ, β, GamesOnNetworks.volume(λ, β))
end
...
struct ScaleFreeModelGenerator <: GraphModelGenerator
    ...
    ScaleFreeModelGenerator(λ::Vector{<:Real}, α::Vector{Float64}) = new(λ, α, GamesOnNetworks.volume(λ, α))
end
