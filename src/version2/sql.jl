...
function insertGameSQL(name::String, payoff_matrix_str::String)
    db = SQLite.DB("SimulationSaves.sqlite")
    result = DBInterface.execute(db, "INSERT OR IGNORE INTO games
                                (
                                    'name',
                                    'payoff_matrix'
                                )
    ...
    tuple_to_return = (status_message = "SQLite [SimulationSaves: games]... INSERT STATUS: [$(result.status.x)] ROW ID: $(result.stmt.id)", insert_row_id = result.stmt.id)
...
function insertGraphSQL(type::String, graph_params_dict_str::String, db_params_dict::Dict{Symbol, Any})
    ...
    result = DBInterface.execute(db, "INSERT OR IGNORE INTO graphs
                                (
    ...
    SQLite.close(db)
    tuple_to_return = (status_message = "SQLite [SimulationSaves: graphs]... INSERT STATUS: [$(result.status.x)] ROW ID: $(result.stmt.id)", insert_row_id = result.stmt.id)
    return tuple_to_return
end
...
function insertSimulationSQL(sim_params_str::String, graph_adj_matrix_str::String, periods_elapsed::Integer, game_id::Integer, graph_id::Integer)
    ...
    println(result.stmt.id)

    SQLite.close(db)

    tuple_to_return = (status_message = "SQLite [SimulationSaves: simulations]... INSERT STATUS: [$(result.status.x)] ROW ID: $(result.stmt.id)", insert_row_id = result.stmt.id)
    return tuple_to_return
end
