...
## 5: run simulation
simulateIterator(game, sim_params_list, graph_params_list; period_cutoff=20000, run_count=1, db_filepath=db_filepath, db_store_period=1000, db_sim_group_id=sim_group_id) #need to create a new group for each time series simulation. Maybe should just make a simulateTimeSeries()? Ultimately can't use simulationIterator with time series stuff
