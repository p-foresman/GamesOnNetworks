...
const sim_group_id_array = [sim_group_id, sim_group_id_2, sim_group_id_3]

## 4: include script that contains all globals for simulation
include("sim_setup_slurm.jl")

slurm_index = ENV["SLURM_ARRAY_TASK_ID"]
three_index = slurm_index % 3
eight_index = slurm_index % 8

sim_group_id = sim_group_id_array[three_index]
sim_params_list = sim_params_list_array[three_index]
graph_params = [graph_params_list[eight_index]]
...
simulationIterator(game, sim_params_list, graph_params, starting_condition, stopping_condition; run_count=20, db_filepath=db_filepath, db_sim_group_id=sim_group_id) # db_filepath=db_filepath, db_sim_group_id=1
